                            ...
                            if not odoo_order_id:
                                if woo_sale_order_dict.get("customer_id") != 0:
                                    ...
                                    if customer_list:  # New Changes by akash
                                        # Handling company wise category
                                        params = {'include' : woo_sale_order_dict.get('line_items', [])[0].get('product_id',0), 'per_page':100}
                                        product_details = wcapi.get('products', params=params)
                                        ...
                                        if company_id.id not in self.env.user.company_ids.ids:
                                            self.env.user.company_ids = [4, company_id.id]
